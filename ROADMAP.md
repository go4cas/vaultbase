...
| E-4 ◐ | **S3 download streaming / presigned redirects** | RAM ceiling closed: `fileResponse` now streams the S3 object (`new Response(s3File.stream())`) instead of buffering the whole thing (`storage.ts`). **Remaining (E-4b):** presigned-URL / `publicUrlFor` redirects to offload the transfer off the server entirely (CDN). | M |
