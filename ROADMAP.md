...
| T-6 ◐ | **Split `auth.ts`** — extracted all shared helpers + constants (IP/lockout, hashing, token/OTP/recovery-code gen, register validation, verify/reset/OTP email issuance) into `api/auth-shared.ts`; `auth.ts` dropped ~1450 → ~1230 lines, no behavior change (full auth suite green). **Deferred:** reshuffling the route handlers into per-concern sub-plugins — the handlers are interleaved by concern on the security-critical surface, so a cosmetic move isn't worth the regression risk. The sub-plugin pattern (`auth-oauth2`, `auth-webauthn`, now `auth-shared`) is established for incremental peel-off when a group is next touched. | `api/auth.ts`, `api/auth-shared.ts` | M |
...
| T-10 | **Extract the rule engine into a reusable `core/rules/` primitive** — *pulled by a concrete second consumer, not built speculatively.* **Update:** F-11b's `waitForEvent` was the expected trigger, but its predicate turned out to be plain JS (the workflow re-runs its own code on resume), so it did *not* need the rule engine. T-10 stays deferred until a real second consumer materializes (e.g. a webhook-condition feature that must evaluate predicates *outside* a code context). | `core/filter.ts`, `core/flags.ts` | M |
...
| E-2 ✅ | **Realtime subscription filters** — a subscriber attaches a `filter` expression to its topics (WS `subscribe` message / SSE POST); events deliver only if the record also matches (AND with `view_rule`), applied to live delivery *and* resume replay. Filter is the client's choice, so admins aren't exempt (new bypass-free `evaluateExpression`). Per-topic, so a broader `*` subscription still delivers. | `realtime/manager.ts`, `core/rules.ts`, `server.ts` | M |
| E-3 ✅ | **SSE/WS resume** (emit `id:`, replay from `Last-Event-ID`) | no event id, no replay on reconnect → gaps lost (`realtime/sse.ts:16`) | M |
| E-4 ✅ | **S3 download streaming + presigned/CDN redirects** | RAM ceiling closed (streams the S3 object). **E-4b done:** opt-in `storage.redirect_downloads` 302-redirects raw downloads to a `public_url` (CDN) or a short-TTL presigned URL — offloading the transfer off the server entirely. Skipped for one-time/IP-bound fields (a URL can't honor per-fetch enforcement) and for thumbnails (server-transformed). Access + audit are enforced *before* the redirect. | `core/storage.ts`, `api/files.ts` | M |
| E-5 ✅ | **Stream uploads to storage** — the upload handler passes the multipart `Blob` straight to `writeFile`; `Bun.write` / the S3 client consume it directly, so there's no second full-file `ArrayBuffer` copy. Local *and* S3. | `api/files.ts`, `core/storage.ts` | S |
...
| F-8 ✅ | **One-off "run at" job scheduling in admin UI** — a one-off is a cron job with an **empty `cron` + future `next_run_at`**; `POST /admin/jobs` now accepts `run_at` (unix seconds, must be future) as an alternative to `cron`. `runJob` gates on empty-cron to fire once and leave `next_run_at=null` (cron-parser treats `""` as "every minute", so the guard is explicit, not a thrown-error side-effect) — verified it never reschedules. Admin editor gained a Recurring/One-off toggle with a `datetime-local` picker (local→UTC), and the jobs list shows `one-off`. Reused the existing cron-jobs table + scheduler rather than the queue delay primitive — no schema change. | `api/jobs.ts`, `core/jobs.ts`, `admin/src/pages/Hooks.tsx` | S–M |
...
| F-10 ✅ | **Per-collection token scopes** — `collection:<name>:<read\|write\|*>` (with `*` wildcards); `read`/`write` stay a global superset. `hasScope` now resolves the grammar, `isValidScope` gates minting. **Also closed a latent gap:** API tokens now actually authenticate on the REST records/batch surface (prefix was never stripped there) *and* are enforced per-collection — so a `["read"]` token can no longer write, and `collection:posts:read` touches nothing but posts. **F-10b (mcp:write split) — already satisfied:** verified the MCP catalog already separates concerns — `mcp:write` gates record writes only, while hooks/routes/settings/flags/webhooks require `mcp:admin`. The only remaining sliver is per-collection *MCP* write scopes (parallel to the REST `collection:*` work); low priority. files/csv API-token access unchanged. | `core/api-tokens.ts`, `api/records.ts`, `api/batch.ts`, `mcp/*` | M |
| F-11 ◐ | **Code-first durable workflows** — engine + `step.run` (memoized) + `step.sleep` + **`step.waitForEvent`** all landed, persisted to `cogworks_workflow_runs`, resumable across restarts (verified). `deliverWorkflowEvent()` + `helpers.workflows.{start,emit}` let hooks/webhooks drive runs (correlation `key` + JS `match` predicate). Memoized-steps model (DBOS/Inngest). **Neat outcome:** because runs re-execute their own code on resume, the `match` predicate is plain JS — so this did **not** need the rule engine, and **T-10 stays unpulled**. **Remaining (F-11c):** admin-authored definitions (compiled like workers) + ops UI, per-run versioning, retry/timeout policy. | `core/workflows.ts`, `core/hook-helpers-extra.ts` | L |
