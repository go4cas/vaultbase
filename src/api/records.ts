...
export function makeRecordsPlugin(jwtSecret: string) {
  return (
    new Elysia({ name: "records" })
      .get(
        "/:collection",
        async ({ params, query, request, set }) => {
          const col = await timeFor(request, "collection_load", () =>
          ...
          const opts: import("../core/records.ts").ListOptions = {
            ...
            perPage: query.perPage ? parseInt(query.perPage, 10) : 30,
            auth,
          };
          ...
          if (query.nearVector && query.nearVectorField) {
            ...
            const vecField = fields.find(
              (f) => f.name === query.nearVectorField && f.type === "vector",
            );
            if (!vecField) {
              set.status = 422;
              return {
                error: `nearVectorField '${query.nearVectorField}' is not a vector field on '${col.name}'`,
                code: 422,
              };
            }
            let queryVec: number[];
            try {
              queryVec = parseVectorParam(query.nearVector);
            } catch (e) {
            ...
            const limit = query.nearLimit
              ? Math.max(1, Math.min(1000, parseInt(query.nearLimit, 10)))
              : 10;
            const ranked = topK({
              query: queryVec,
      ...
      .get("/:collection/:id", async ({ params, request, set }) => {
        ...
        const auth = await getAuthContext(request, jwtSecret);
        const record = await timeFor(request, "db_exec", () =>
          getRecord(params.collection, params.id),
        );
        if (!record) {
          set.status = 404;
      ...
      .delete("/:collection/:id", async ({ params, request, set }) => {
        ...
        try {
          await runWithHookRequest(request, () => deleteRecord(params.collection, params.id, auth));
        } catch (e) {
          if (e instanceof ValidationError) return validationResponse(set, e);
          if (e instanceof ReadOnlyCollectionError) return readOnlyResponse(set, e);
      ...
      .get("/:collection/:id/history", async ({ params, query, request, set }) => {
        ...
        const out: HistoryListResponse = await listRecordHistory(params.collection, params.id, {
          perPage,
          page,
        });
        return { data: out };
