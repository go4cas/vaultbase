...
export async function listAdminSessions(
  ...
  const rows = opts.activeOnly
    ? await db
        .select()
        .from(adminSessions)
        .where(gte(adminSessions.expires_at, now))
        .orderBy(desc(adminSessions.issued_at))
...
export function getLockoutPolicy(): LockoutPolicy {
  ...
  const dur = Math.max(
    60,
    Number.parseInt(getSetting("auth.lockout.duration_seconds", "900"), 10) || 900,
  );
...
export async function recordLoginFailure(opts: {
  ...
  if (opts.email)
    rows.push({ id: crypto.randomUUID(), key: `email:${opts.email.toLowerCase()}`, at: now });
...
export async function clearLoginFailures(opts: {
  email?: string;
  ip?: string | null;
}): Promise<void> {
  const db = getDb();
  if (opts.email)
    await db
      ...
      .where(eq(loginFailures.key, `email:${opts.email.toLowerCase()}`));
...
export async function isLockedOut(opts: { email: string; ip: string | null }): Promise<boolean> {
  ...
  for (const k of [`email:${opts.email.toLowerCase()}`, opts.ip ? `ip:${opts.ip}` : null]) {
    ...
    const rows = await db
      .select()
      .from(loginFailures)
      .where(and(eq(loginFailures.key, k), gte(loginFailures.at, cutoff)));
