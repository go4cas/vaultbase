...
function evaluateEach(left: Operand, right: Operand, op: AnyCmpOp, ctx: EvalContext): boolean {
  ...
  return arr.every((item) => {
    switch (op) {
      case "=":
        return looseEq(item, target);
      case "!=":
        return !looseEq(item, target);
      case ">":
        return cmp(item, target) > 0;
      case ">=":
        return cmp(item, target) >= 0;
      case "<":
        return cmp(item, target) < 0;
      case "<=":
        return cmp(item, target) <= 0;
      case "~":
        return String(item ?? "").includes(String(target ?? ""));
      case "!~":
        return !String(item ?? "").includes(String(target ?? ""));
      default:
        return false;
...
function evaluateArrayCmp(
  ...
  for (const item of arr) {
    ...
    switch (scalarOp) {
      case "=":
        match = looseEq(item, target);
        break;
      case "!=":
        match = !looseEq(item, target);
        break;
      case ">":
        match = cmp(item, target) > 0;
        break;
      case ">=":
        match = cmp(item, target) >= 0;
        break;
      ...
      case "<=":
        ...
        break;
      case "~":
        match = String(item ?? "").includes(String(target ?? ""));
        break;
      case "!~":
        match = !String(item ?? "").includes(String(target ?? ""));
        break;
...
function resolveOperand(op: Operand, ctx: EvalContext): unknown {
  switch (op.kind) {
    ...
    case "requestMap": {
      ...
      const v =
        (map as Record<string, unknown> | undefined)?.[norm] ??
