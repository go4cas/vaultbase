...
describe("generateThumbnail — WebP", () => {
  it("decodes a 200x200 source, resizes to ~50x50, emits valid WebP", async () => {
    ...
    const decoded = await webpDecode(
...
describe("generateThumbnail — AVIF", () => {
  it("decodes a 32x32 source, resizes to 16x16, emits valid AVIF", async () => {
    ...
    const decoded = await avifDecode(
      thumb.buffer.slice(thumb.byteOffset, thumb.byteOffset + thumb.byteLength) as ArrayBuffer,
    );
...
describe("generateThumbnail — animated GIF", () => {
  it.skipIf(!gifEncoderAvailable)(
    "preserves multi-frame structure when thumbnailing an animated source",
    async () => {
...
describe("generateThumbnail — static GIF", () => {
  it.skipIf(!gifEncoderAvailable)(
    "returns non-empty bytes that respect contain dims (downgrade to PNG is fine)",
    async () => {
