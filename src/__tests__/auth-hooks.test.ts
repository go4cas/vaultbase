...
describe("auth signup hook lifecycle", () => {
  it("/register fires beforeCreate + afterCreate", async () => {
    ...
    await installHook(
      "users",
      "beforeCreate",
      `globalThis._beforeHits = (globalThis._beforeHits || 0) + 1;`,
    );
    await installHook(
      "users",
      "afterCreate",
      `globalThis._afterHits  = (globalThis._afterHits  || 0) + 1;`,
    );
  ...
  it("/anonymous fires afterCreate", async () => {
    ...
    await installHook(
      "users",
      "afterCreate",
      `globalThis._anonHits = (globalThis._anonHits || 0) + 1;`,
    );
  ...
  it("global hooks (collection_name='') fire on auth signup too", async () => {
    ...
    await installHook(
      "",
      "afterCreate",
      `globalThis._globalHits = (globalThis._globalHits || 0) + 1;`,
    );
