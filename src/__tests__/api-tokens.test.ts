...
describe("mintApiToken", () => {
  ...
  it("rejects invalid input", async () => {
    ...
    await expect(
      mintApiToken(
        { name: "", scopes: ["read"], createdBy: me.id, createdByEmail: me.email },
        SECRET,
      ),
    ).rejects.toThrow(/name is required/);
    await expect(
      mintApiToken({ name: "ok", scopes: [], createdBy: me.id, createdByEmail: me.email }, SECRET),
    ).rejects.toThrow(/scope/);
    await expect(
      mintApiToken(
        { name: "x".repeat(101), scopes: ["read"], createdBy: me.id, createdByEmail: me.email },
        SECRET,
      ),
...
describe("verifyAuthToken — audience=api", () => {
  ...
  it("rejects when audience mismatched", async () => {
    ...
    const wrongAud = await verifyAuthToken(stripApiTokenPrefix(r.token), SECRET, {
      audience: "admin",
    });
  ...
  it("rejects when minting admin's password_reset_at is bumped (force-logout-all kills API tokens too)", async () => {
    ...
    await getDb()
      .update(admin)
      .set({ password_reset_at: Math.floor(Date.now() / 1000) + 60 })
      .where(eq(admin.id, me.id));
    const ctx = await verifyAuthToken(stripApiTokenPrefix(r.token), SECRET, { audience: "api" });
    expect(ctx).toBeNull();
  });
});
...
describe("listApiTokens", () => {
  it("returns rows ordered by created_at desc", async () => {
    const me = await seedAdmin();
    await mintApiToken(
      { name: "first", scopes: ["read"], createdBy: me.id, createdByEmail: me.email },
      SECRET,
    );
    await new Promise((r) => setTimeout(r, 1100)); // unix-second granularity
    await mintApiToken(
      { name: "second", scopes: ["write"], createdBy: me.id, createdByEmail: me.email },
      SECRET,
    );
    const rows = await listApiTokens();
