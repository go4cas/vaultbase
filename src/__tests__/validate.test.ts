...
describe("validateRecord — text constraints", () => {
  ...
  it("enforces max length", async () => {
    ...
    await expect(validateRecord(col, { title: "abcdef" }, "create")).rejects.toThrow(
      ValidationError,
    );
  ...
  it("enforces regex pattern", async () => {
    ...
    await expect(validateRecord(col, { slug: "hello world" }, "create")).rejects.toThrow(
      ValidationError,
    );
...
describe("validateRecord — email", () => {
  it("rejects bad emails", async () => {
    ...
    await expect(validateRecord(col, { email: "not-an-email" }, "create")).rejects.toThrow(
      ValidationError,
    );
    await expect(validateRecord(col, { email: "user@" }, "create")).rejects.toThrow(
      ValidationError,
    );
    await expect(
      validateRecord(col, { email: "user@example.com" }, "create"),
...
describe("validateRecord — url", () => {
  it("rejects bad URLs", async () => {
    ...
    await expect(validateRecord(col, { site: "example.com" }, "create")).rejects.toThrow(
      ValidationError,
    );
    await expect(validateRecord(col, { site: "ftp://x.com" }, "create")).rejects.toThrow(
      ValidationError,
    );
    await expect(
      validateRecord(col, { site: "https://example.com" }, "create"),
    ).resolves.toBeUndefined();
    await expect(
      validateRecord(col, { site: "http://example.com/path?q=1" }, "create"),
    ).resolves.toBeUndefined();
  });
});
...
describe("validateRecord — select", () => {
  it("rejects values not in allowed list", async () => {
    ...
    await expect(validateRecord(col, { status: "published" }, "create")).rejects.toThrow(
      ValidationError,
    );
  ...
  it("multiple: validates each value", async () => {
    const col = makeCol([
      { name: "tags", type: "select", options: { values: ["a", "b", "c"], multiple: true } },
    ]);
    await expect(validateRecord(col, { tags: ["a", "x"] }, "create")).rejects.toThrow(
      ValidationError,
    );
  ...
  it("rejects any value when select has no allowed values configured", async () => {
    ...
    await expect(validateRecord(col, { status: "anything" }, "create")).rejects.toThrow(
      ValidationError,
    );
  });
});
